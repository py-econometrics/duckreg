...
For dense balanced panels with binary treatment and binary or low-cardinality outcomes, [Bitpacked Panels](bitpacked-mundlak.qmd) documents the specialized bitmap estimators. Those estimators compute the same Mundlak-style moments from packed unit-major and time-major words.
...
| `BitmapMundlak` | Pack dense low-cardinality panels into bitmaps, then compute static Mundlak moments directly. |
| `BitmapMundlakEventStudy` | Combine cohort masks with time-major outcome bitplanes to compute cohort-time event-study moments. |
