"""Bitmap moment estimators for dense low-cardinality panel data.

This module stores dense balanced panels as packed 64-bit words and computes
low-dimensional regression moments with database popcount operations.  The
current SQL implementation targets DuckDB because Ibis does not currently
...
class BitmapMundlak(_BitmapEstimator):
    ...
    def from_long(cls, data: pd.DataFrame, **kwargs):
        ...
        panel = LongPanelBitPacker(**packer_kwargs).fit_transform(data)
        estimator_kwargs = {
            key: value
    ...
    def compress_data(self):
        sql = _with_popcount_function(
            STATIC_MUNDLAK_DIRECT_SQL, self.popcount_function
        )
...
class BitmapMundlakEventStudy(_BitmapEstimator):
    ...
    def compress_data(self):
        sql = _with_popcount_function(COHORT_TIME_SUMS_SQL, self.popcount_function)
        with _bitmap_connection(self.panel) as con:
    ...
    def estimate(self) -> dict[str, pd.DataFrame]:
        ...
        n_lookup = {
            ...
            for row in self.cohort_time.drop_duplicates("cohort").itertuples(index=False)
        }
...
def fit_static_mundlak_bitmap(
    ...
    panel = pack_long_panel_to_bitmaps(
        data,
        unit_col=unit_col,
        time_col=time_col,
        outcome_col=outcome_col,
...
def fit_static_mundlak_bitmap_panel(
    ...
    return BitmapMundlak(panel, treatment_col=treatment_col).fit().point_estimate
...
def fit_event_study_bitmap(
    ...
    return fit_event_study_bitmap_panel(
        panel,
        pre_treat_interactions=pre_treat_interactions,
    )
