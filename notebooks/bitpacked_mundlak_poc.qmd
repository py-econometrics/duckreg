...
The same representation also makes unit-cluster bootstrap inference straightforward. A bootstrap draw is a multiplicity vector $m_i$ over units. The static estimator recomputes the unit-additive pieces by weighted sums over units and recomputes the time margins as
...
The event-study estimator uses the same multiplicities to recompute cohort-time cell counts and outcome sums. In neither case does the bootstrap materialize a resampled long table or rerun the long-format groupby compression.
