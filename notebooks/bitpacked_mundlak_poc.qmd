...
The coverage experiment uses the same staggered-adoption DGP as the static inference example: $N=10{,}000$, $T=30$, cohorts $(10, 15, 20)$, and treated counts $(2{,}000, 3{,}000, 2{,}000)$. Each Monte Carlo replication uses analytic unit-cluster CRV1 inference for `pyfixest` and $B=200$ full-sample unit bootstrap replicates for each compressed estimator.

The DGP has cohort- and event-time-specific effects, so the static coefficient is a projection estimand. For each replication, the target below is the coefficient on $W_{it}$ from the static Mundlak projection with $Y_{it}$ replaced by the DGP probability $\mathbb E[Y_{it} \mid W, \alpha_i, \lambda_t]$.
...
coverage_path = (
    repo_root
    / "notebooks"
    / "bitpacked_mundlak_coverage_R1000_N10000_B200.csv"
)
coverage_draws = pd.read_csv(coverage_path)
coverage_replications = len(coverage_draws)
    ...
    [
        {
            ...
            "mean_seconds": coverage_draws["pyfixest_seconds"].mean(),
        },
        {
            ...
            "mean_seconds": coverage_draws["dbmundlak_seconds"].mean(),
        },
        {
            ...
            "mean_seconds": coverage_draws["bitmap_seconds"].mean(),
        },
    ]
)
